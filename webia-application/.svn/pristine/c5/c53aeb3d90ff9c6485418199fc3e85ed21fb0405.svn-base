...
@Service
public class WithdrawalDocumentGenerationService extends PolicyDocumentService implements PolicyBasedDocumentGenerationService, TransactionFormBasedDocument {
	...
	private static final Integer EVENT = 15;
	...
	private Endorsement generateEndorsement(PolicyDTO policyDTO, String productCountry, Long workflowItemId) {
		...
		Collection<FundTransactionDTO> fundTransactions = transactions.stream().map(x -> x.getFundTransactions()).flatMap(Set::stream).filter(x -> x.getEventType() == 15).collect(Collectors.toList());
	...
	private SurrenderDetails generateSurrenderDetails(Collection<FundTransactionDTO> fundTransactions, TransactionFormDTO transactionFormDTO, PolicyDTO policyDTO,
		...
		TransactionTaxDTO transactionTaxSelected = getTranDTO(policyDTO, transactionFormDTO.getEffectiveDate());
		...
		surrenderDetails.setNettoAmount(getSurrenderAmout(transactions));

		// TODO surrenderAmount
		surrenderDetails.setAmount(generateBruttoAmount(surrenderDetails));

		// contract management fees
		/*
		 * TODO use the transactionLinked field to get the linked transactions(Eventtype 12 et 13) add all the fees(column value0)
		 */
		...
		surrenderDetails.setFiscalDataFrance(generateFiscalDataFrance(policyDTO, transactionFormDTO.getEffectiveDate()));
	...
	private AmountType generateBruttoAmount(SurrenderDetails surrenderDetails) {
		AmountType feeAmount = new AmountType();
		BigDecimal value = surrenderDetails.getNettoAmount().getValue().add(surrenderDetails.getSurrenderFeesAmount().getValue());
		feeAmount.setValue(value);
		feeAmount.setCurrencyCode(surrenderDetails.getNettoAmount().getCurrencyCode());

		return feeAmount;
	}

	private FiscalDataFrance generateFiscalDataFrance(PolicyDTO policyDTO, Date effectiveDate) {

		if (frenchTaxHelper.isPolicyFrenchTaxable(policyDTO.getPolId()).isPresent()) {
			TransactionTaxDTO TransactionTaxSelected = frenchTaxHelper.getTransactionTax(policyDTO.getPolId(), effectiveDate,true);
			return surrenderDocumentGenerationServiceCommonImpl.generateFiscalDataFrance(TransactionTaxSelected);
		} else {		
			frenchTaxHelper.getTransactionTax(policyDTO.getPolId(), effectiveDate,false);
			return null;
		}
			
	}

	private TransactionTaxDTO getTranDTO(PolicyDTO policyDTO, Date effectiveDate) {
	...
	private AmountType generateSurrenderFeesAmount(Collection<TransactionDTO> transactions) {
		...
		if (transactions.size() > 0) {
			feeAmount.setValue(generateSurrenderFee(transactions));
			feeAmount.setCurrencyCode(transactions.stream().findAny().get().getCurrency());
		}
		return feeAmount;
	}

	private BigDecimal generateSurrenderFee(Collection<TransactionDTO> transactions) {
	...
	private AmountType getSurrenderAmout(Collection<TransactionDTO> transactions) {
		AmountType amount = new AmountType();
		BigDecimal result = transactions.stream().map(x -> x.getValue0()).reduce(BigDecimal.ZERO, BigDecimal::add);
		amount.setValue(result.abs());
		amount.setCurrencyCode(transactions.stream().findAny().get().getCurrency());
		return amount;
	}
	...
	protected DocumentType getDocumentType() {
		return DocumentType.WITHDRAWAL;
	}
